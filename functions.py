...
def country_event_heatmap(df,country):
    ...
    pt = new_df.pivot_table(index='Sport', columns='Year', values='Name', aggfunc='count').fillna(0)
    return pt

def country_event_heatmap2(df,country):
    temp_df = df.dropna(subset=['Medal'])
    temp_df.drop_duplicates(subset=['Team', 'NOC', 'Games', 'Year', 'City', 'Sport', 'Event', 'Medal'], inplace=True)
...
def success(df,sport,event):
    temp_df = df.dropna(subset=['Medal'])



    if (sport=='Overall') and (event=='Overall'):
        temp_df=temp_df
    if (sport=='Overall') and (event!='Overall'):
